...
const ProductDetails = () => {
  ...
  const categoriesFromState = useSelector((state) => state?.categoryState?.categories);
  const categories = useMemo(() => categoriesFromState || content?.categories || [], [categoriesFromState]);
  const similarProducts = useMemo(() => {
    if (!product || !content?.products) {
      return [];
    }
    ...
    const filtered = content.products.filter(p =>
      p.category_id === currentCategoryId && p.id !== currentProductId
    );
  ...
  const breadCrumbLinks = useMemo(() => {
    ...
    if (productCategory) {
     ...
     if (productType) {
       links.push({ title: productType.name });
     }
    if (product) {
      links.push({ title: product.title });
    }
    return links;
  ...
  */
  ...
  const handleAddToCart = useCallback(() => {
    ...
    if (product.size?.includes(selectedSize)) {
        ...
        console.log("Added basic product to cart:", product.title, selectedSize);
    } else {
         setError('Kích cỡ không hợp lệ.'); 
    }
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div className='flex flex-col md:flex-row gap-8 lg:gap-16'>
          ...
          <div className='flex flex-col-reverse md:flex-row gap-4'>
            ...
            <div className='flex md:flex-col gap-3 justify-center md:justify-start flex-wrap md:flex-nowrap md:w-auto'>
              <button
                ...
                >
                  <img src={product.thumbnail} className='h-16 w-16 lg:h-20 lg:w-20 object-cover cursor-pointer transition duration-150 ease-in-out' alt={`Thumbnail chính`} />
              </button>
              {productImages.map((imgUrl, index) => (
                <button key={index} onClick={() => setCurrentImage(imgUrl)} className={`rounded-lg overflow-hidden border-2 p-0.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${currentImage === imgUrl ? 'border-indigo-600' : 'border-transparent'} hover:border-gray-300`} aria-label={`Xem ảnh ${index + 1}`}>
                  <img src={imgUrl} className='h-16 w-16 lg:h-20 lg:w-20 object-cover cursor-pointer transition duration-150 ease-in-out' alt={`${product.title} - Ảnh ${index + 1}`} />
                </button>
              ))}
            </div>
            <div className='flex-1'>
              <img src={currentImage || product.thumbnail} className='w-full aspect-square rounded-xl border border-gray-200 object-cover shadow-sm' alt={product.title} />
            </div>
          </div>
        </div>
        ...
        <div className='w-full md:w-1/2 lg:w-7/12'>
          <h1 className='text-2xl sm:text-3xl font-bold tracking-tight text-gray-900'>{product.title}</h1>
          {product.rating && (<div className="mt-3"><Rating rating={product.rating} /></div>)}
          <div className='mt-4 flex items-baseline gap-x-2'>
             <p className='text-3xl font-bold tracking-tight text-gray-900'>${product.price?.toFixed(2)}</p>
             {product.discount > 0 && product.price && (<p className='text-lg text-gray-500 line-through'>${(product.price / (1 - product.discount / 100)).toFixed(2)}</p>)}
          </div>
          {availableSizes.length > 0 && (
            <div className='mt-8'>
              <div className='flex justify-between items-center mb-3'>
                <p className='text-base font-medium text-gray-900'>Chọn Kích Cỡ</p>
                <Link className='text-sm font-medium text-indigo-600 hover:text-indigo-500' to={'/size-guide'} target='_blank'>Hướng dẫn chọn size</Link>
              </div>
              <SizeFilter sizes={availableSizes} selectedSize={selectedSize} onChange={(size) => setSelectedSize(size)} multi={false}/>
      ...
      {product.description && (
        <div className="mt-12 lg:mt-16 pt-10 border-t border-gray-200">
          ...
          <div className='mt-6 prose prose-gray max-w-none text-gray-600 prose-sm sm:prose-base'><p>{product.description}</p></div>
        </div>
      )}
      ...
      <div className="mt-12 lg:mt-16 pt-10 border-t border-gray-200">
        ...
        <div className='mt-8 grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 lg:grid-cols-4 sm:gap-x-6 lg:gap-x-8 lg:gap-y-10'>
          {similarProducts.length > 0 ? (
             similarProducts.map((item) => (<ProductCard key={item.id} {...item} title={item.title} />))
          ) : (
